...
int main(int argc, char **argv) 
{
	...
	if(rank==0)
	{
		...
		{
			MPI_Send(&A[i*n*m/processes],n*m/processes , MPI_FLOAT, i, 0, MPI_COMM_WORLD);
			MPI_Send(B,m*n , MPI_FLOAT, i, 0, MPI_COMM_WORLD);
		}		 
	}
	else
	{
		MPI_Recv(&A[rank*n*m/processes],n*m/processes , MPI_FLOAT, 0, 0, MPI_COMM_WORLD, 0);
		MPI_Recv(B,m*n , MPI_FLOAT, 0, 0, MPI_COMM_WORLD, 0);
	}
